...
public class Bvh implements AccelStruct {   
	...
	private boolean intersectHelper(BvhNode node, IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection)
	{
		...
		if (node.isLeaf()) {
			...
			leftRay.makeOffsetRay();
			rightRay.makeOffsetRay();
	...
	private BvhNode createTree(int start, int end) {
		...
		Vector3d minBound = surfaces[start].getMinBound();
		Vector3d maxBound = surfaces[start].getMaxBound();
		...
		return new BvhNode(minBound, maxBound, createTree(start, midPt), createTree(midPt, end), start, end);
	}
}
